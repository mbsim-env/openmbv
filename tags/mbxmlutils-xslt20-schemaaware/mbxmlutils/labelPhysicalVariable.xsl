...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="element(*,pv:scalar)">
    <xsl:copy>
      @SCALAREXPRESSION{@<xsl:value-of select="normalize-space(.)"/>@}@
    </xsl:copy>
  </xsl:template>

  <xsl:template match="element(*,pv:vector)">
    <xsl:copy>
      @VECTOREXPRESSION{@<xsl:value-of select="normalize-space(.)"/>@}@
    </xsl:copy>
  </xsl:template>

  <xsl:template match="element(*,pv:matrix)">
    <xsl:copy>
      @MATRIXEXPRESSION{@<xsl:value-of select="normalize-space(.)"/>@}@
    </xsl:copy>
  </xsl:template>
