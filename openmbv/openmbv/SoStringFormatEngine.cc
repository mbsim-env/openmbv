...
void StringFormatEngine::evaluate() {
  int size = format.getNum();
  currentFormat.resize(size);
  convertedFormat.resize(size);
  SO_ENGINE_OUTPUT(output, SoMFString, setNum(size));
  for(int j=0; j<size; ++j) {
    std::string formatStr = format[j].getString();
    if(formatStr != currentFormat[j]) {
      currentFormat[j] = formatStr;
      ...
      convertedFormat[j] = formatStr+"%"+std::to_string(keys.size()+1)+"%";
      for(size_t i=0; i<keys.size(); ++i) {
        boost::algorithm::replace_all(convertedFormat[j], "%"+keys[i]+"%" , "%"+std::to_string(i+1)+"%");
        boost::algorithm::replace_all(convertedFormat[j], "%"+keys[i]+"$" , "%"+std::to_string(i+1)+"$");
        boost::algorithm::replace_all(convertedFormat[j], "%|"+keys[i]+"$", "%|"+std::to_string(i+1)+"$");
      }
    }
    ...
    auto outStr = (boost::format(convertedFormat[j]) %
    ...
    #undef V
    #undef S
    SO_ENGINE_OUTPUT(output, SoMFString, set1Value(j, outStr.data()));
  }
}
