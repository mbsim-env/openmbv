...
GearWheel::GearWheel(const std::shared_ptr<OpenMBV::Object> &obj, QTreeWidgetItem *parentItem, SoGroup *soParent, int ind) : RigidBody(obj, parentItem, soParent, ind) {
  ...
  double c = 0.167*m;
  double s0 = p0/2;
  double df = d0 - sign*2*(m+c);
  double da = d0 + sign*2*m;
  ...
  double sb = db*(s0/d0+sign*phi0)-b;
  double deb = 2*sb/db;
  double ga0 = 2*p0/d0;
  double rb = db/2;
  double ra = da/2;
  double rf = df/2;
  int numb = 5;
  double dphi = (solid?(ga0-deb)/2:(phif-deb/2))/numb;
  double phi = -ga0/2;
  ...
  phi = solid?0:(phif+alf);
  for (int i=numb; i<2*numb; i++) {
    x[i] = rb*(cos(-deb/2)*sign*(sin(phi)-cos(phi)*phi) + sin(-deb/2)*(cos(phi)+sin(phi)*phi));
    y[i] = rb*(-sin(-deb/2)*sign*(sin(phi)-cos(phi)*phi) + cos(-deb/2)*(cos(phi)+sin(phi)*phi));
    phi += dphi;
  }
  dphi = (solid?(deb/2-phia):deb/2)/numb;
  phi = solid?(phia-deb/2):-deb/2;
  double R = solid?ra:rb;
