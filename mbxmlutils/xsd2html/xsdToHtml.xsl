<xsl:stylesheet
  ...
  <!-- all nodes of all additional schemas files and myself (addinput.xml is generated by mbxmlutils-xsd2html) -->
  <xsl:param name="ALLNODES" select="document(document(concat($PWD, '/addinput.xml'))/root/a/text())|/"/>
  ...
  <xsl:template match="/">
    ...
    Generated on <xsl:value-of select="$DATETIME"/> for <xsl:value-of select="$PROJECT"/> by MBXMLUtils <a href="https://validator.w3.org/check?uri=referer"><img src="https://www.w3.org/Icons/valid-html401-blue.png" alt="Valid HTML"/></a></span>
    </body></html>
  </xsl:template>
